...
use "$data\ehcvm_conso_${iso3}2018.dta", clear
...
use "$data\s13a_2_me_${iso3}2018.dta", clear
...
use "$data\s13b_2_me_${iso3}2018.dta", clear
gen double hhid = 1000*grappe + menage
if "${iso3}" == "CIV" | "${iso3}" == "TGO" | "${iso3}" == "GIN" replace hhid = 100 * grappe + menage
if "${iso3}" == "GNB" replace hhid = 100 * grappe + menage if menage < 10
...
collapse (sum) remit_sent, by(hhid grappe menage)
...
use "$data\s05_me_${iso3}2018.dta", clear
gen double hhid = 1000*grappe + menage 
if "${iso3}" == "CIV" | "${iso3}" == "TGO" | "${iso3}" == "GIN" replace hhid = 100 * grappe + menage
if "${iso3}" == "GNB" replace hhid = 100 * grappe + menage if menage < 10
...
use "${data}\ehcvm_individu_${iso3}2018.dta", clear
...
use "$data\ehcvm_welfare_${iso3}2018.dta", clear
...
keep idh wave_* rural region_* hhsize pcc ipcc wgt zref noncons_exp nonlabor_income consumption
...
save "$temp\MPO_hh_${iso3}.dta", replace
...
use "$data\s02_me_${iso3}2018.dta", clear
