...
*putexcel set "${toolB}\main_results.xlsx", sheet($iso3) modify
*putexcel W4 = $icp
*putexcel W5 = $cpi
...
*graph export "${graphs}\li_cons_${iso3}.png", replace
*putexcel P11 = image("${graphs}\\li_cons_${iso3}.png")
...
preserve
collapse (sum) li , by(idh idi branch csp rural female)

gen salaried=1 if csp==1|csp==2|csp==3|csp==4|csp==5|csp==6
gen self_employee=1 if csp==7|csp==8|csp==9|csp==10
gen salaried_income=li if salaried==1
gen self_employee_income=li if self_employee==1

scalar tot_self_income=sum(self_employee_income)
scalar tot_employee_income=sum(salaried_income)
